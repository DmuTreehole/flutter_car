...
class _MainPageState extends State<MainPage> {
  ...
  _send(String buf) async {
    ...
    var dataLength = await sender.send(
        buf.codeUnits, Endpoint.unicast(InternetAddress('192.168.1.1'),port: Port(serport)));
    log("$dataLength bytes sent.");
    sender.close();
    /*
    RawDatagramSocket rawDgramSocket =
        await RawDatagramSocket.bind('127.0.0.1', 8080);
    rawDgramSocket.send(
        utf8.encode(buf), InternetAddress('192.168.1.1'), 7895);
        */
  }
